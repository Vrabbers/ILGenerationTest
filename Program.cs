...
namespace ILGenerationTest
{
    class Program
    {
        static void Main(string[] argg)
        {
            ...
            var asm = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("Brainfuck.NET Assembly"), AssemblyBuilderAccess.Run);
            var module = asm.DefineDynamicModule("Brainfuck.NET Module");
            var type = module.DefineType("BrainfuckNETOutput", TypeAttributes.Class | TypeAttributes.Sealed, null, new[]{IBrainfuck.Type});
            var start = type.DefineMethod(nameof(IBrainfuck.Do), MethodAttributes.Public | MethodAttributes.Virtual, null, null);
            type.DefineMethodOverride(start, IBrainfuck.Type.GetMethod(nameof(IBrainfuck.Do)) ?? throw new ArgumentNullException());
            ...
            else throw new MissingMemberException("Pee");
    ...
    {
        public static Type Type = typeof(IBrainfuck);
        public void Do();
    }
}